...
{
    public const string Version = "4.1.4.0";
    ...
    private static readonly string NuGetDefenseFolder =
        Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData,
                Environment.SpecialFolderOption.Create), ".nugetDefense");

    /// <summary>
    ///     The global vulnerability data used by NuGetDefense
    /// </summary>
    public static readonly string VulnerabilityDataBin =
        Path.Combine(NuGetDefenseFolder, DefaultVulnerabilityDataFileName);
    ...
    private void LoadSettings(ScanOptions options)
    {
        try
        {
            if (options.SettingsFile == null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(options.ProjectFile?.DirectoryName))
                {
                    var projectSettingsPath =
                        Path.Combine(options.ProjectFile?.DirectoryName!, DefaultSettingsFileName);
                    // Check Project Directory First for Settings
                    if (File.Exists(projectSettingsPath))
                    {
                        Log.Logger.Information(
                            $"NuGetDefense: Using project level settings file at: {projectSettingsPath}");

                        settingsFilePath = projectSettingsPath;
                    }
                    else if (File.Exists(Path.Combine(Directory.GetParent(projectSettingsPath)?.Parent?.FullName ?? "",
                                 DefaultSettingsFileName)))
                    {
                        // Use Parent Directory of Project if the Settings File exists there instead
                        settingsFilePath = Path.Combine(Directory.GetParent(projectSettingsPath)?.FullName ?? "",
                            DefaultSettingsFileName);
                        Log.Logger.Information(
                            $"NuGetDefense: Using project level settings file at parent folder: {settingsFilePath}");
                    }
                }
            ...
            {
                Log.Logger.Information(
                    $"NuGetDefense: Using argument-driven settings file at: {options.SettingsFile.FullName}");
    ...
    private void ScanVulnerabilities(ScanOptions options)
    {
        try
        {
            ...
            if (options.ProjectFile != null &&
                options.ProjectFile.Extension.Equals(".sln", StringComparison.OrdinalIgnoreCase))
            {
                var projects = DotNetSolution.Load(projectFullName).Projects.Where(p => !p.Type.IsSolutionFolder)
            ...
            else
            {
                ...
                Log.Logger.Information("Target Framework: {framework}",
                    string.IsNullOrWhiteSpace(options.Tfm) ? "Undefined" : options.Tfm);
                Log.Logger.Verbose("Loading Packages");
                Log.Logger.Verbose("Transitive Dependencies Included: {CheckTransitiveDependencies}",
                    _settings.CheckTransitiveDependencies);

                if (_settings.CheckTransitiveDependencies && nugetFile.PackagesConfig)
                {
                    var projects = DotNetProject.Load(projectFullName).ProjectReferences.Select(p => p.FilePath)
                        .ToArray();
                    var specificFramework = !string.IsNullOrWhiteSpace(options.Tfm);
                    if (specificFramework) Log.Logger.Information("Target Framework: {framework}", options.Tfm);
                ...
                {
                    _projects = new();
                    _projects.Add(nugetFile.Path,
                        nugetFile.LoadPackages(options.Tfm, _settings.CheckTransitiveDependencies).Values.ToArray());
                }
            }
            ...
            if (_settings.ErrorSettings.IgnoredPackages is { Length: > 0 })
                ...
                {
                    UtilityMethods.IgnorePackages(in packages, _settings.ErrorSettings.IgnoredPackages,
                        out var projPackages);
            ...
            if (_settings.OssIndex.Enabled)
            {
                ...
                var uncachedPkgs = options.Cache.GetUncachedPackages(nonSensitivePackageIDs, TimeSpan.FromDays(1),
                    ossIndexSourceId, out var cachedPackages);
                ...
                if (modUncached > 0 && cachedPackages.Length > 0)
                {
                    ...
                    for (var i = cachedPackages.Length - 1; i >= 0; i--) uncachedPkgs.Add(cachedPackages[i]);
                ...
                vulnDict =
                    new OSSIndex.Scanner(_nuGetFile, _settings.OssIndex.BreakIfCannotRun, UserAgentString,
                            _settings.OssIndex.Username, _settings.OssIndex.ApiToken)
                ...
                options.Cache.GetPackagesCachedVulnerabilitiesForSource(cachedPackages.Skip(uncachedPkgs.Count % 128),
                    ossIndexSourceId, ref vulnDict);
            }
            ...
            if (_settings.GitHubAdvisoryDatabase.Enabled)
            {
                ...
                {
                    var msBuildMessage = MsBuild.Log(_nuGetFile,
                        _settings.GitHubAdvisoryDatabase.BreakIfCannotRun
                            ? MsBuild.Category.Error
                            : MsBuild.Category.Warning,
                ...
                else
                {
                    ...
                    var uncachedPkgs = options.Cache.GetUncachedPackages(nonSensitivePackageIDs, TimeSpan.FromDays(1),
                        gitHubAdvisoryDatabaseSourceId, out var cachedPackages);
                    var modUncached = uncachedPkgs.Count % 128;
                    if (modUncached > 0 && cachedPackages.Length > 0)
                    {
                        var rescanCount = Math.Min(128 - modUncached, cachedPackages.Length);
                    ...
                    var ghsaVulnDict =
                        new GitHubAdvisoryDatabase.Scanner(_nuGetFile, _settings.GitHubAdvisoryDatabase.ApiToken,
                                _settings.GitHubAdvisoryDatabase.BreakIfCannotRun)
                            .GetVulnerabilitiesForPackages(uncachedPkgs.ToArray());
                    ...
                    options.Cache.GetPackagesCachedVulnerabilitiesForSource(cachedPackages,
                        gitHubAdvisoryDatabaseSourceId, ref vulnDict);
                }
            }
            ...
            if (_settings.NVD.Enabled)
            {
                ...
                foreach (var (_, pkgs) in _projects)
                    vulnDict =
                        new NVD.Scanner(_nuGetFile, VulnerabilityDataBin,
                                TimeSpan.FromSeconds(_settings.NVD.TimeoutInSeconds),
    ...
    private async Task<Dictionary<string, Dictionary<string, VulnerabilityEntry>>> ReadFromBinFileAsync(
        string vulnDataFile)
    {
        ...
        do
        {
            try
            {
                ...
                return await MessagePackSerializer
                    .DeserializeAsync<Dictionary<string, Dictionary<string, VulnerabilityEntry>>>(nvdData, lz4Options);
            }
            catch (Exception e)
            {
    ...
    private void MergeVulnDict(ref Dictionary<string, Dictionary<string, Vulnerability>> vulnDict,
        ref Dictionary<string, Dictionary<string, Vulnerability>> vulnDict2)
    {
        ...
        bool solutionFile = false)
    {
        ...
        foreach (var project in projects)
        {
            ...
            {
                pkgs.AddDistinctPackages(nugetFile.LoadPackages(targetFramework, _settings.CheckTransitiveDependencies)
                    .Values);
            }
        ...
        {
            var packages = topLevelNuGetFile.LoadPackages(targetFramework, _settings.CheckTransitiveDependencies)
                .Values;
    ...
    {
        nonSensitives = new();
        var sensitiveRegexSet =
            _settings.SensitivePackages.Select(sp => Regex.Escape(sp).Replace(@"\*", ".*")).ToArray();
        foreach (var (project, packages) in _projects)
            nonSensitives.Add(project,
                packages.Where(p => !sensitiveRegexSet.Any(x => Regex.IsMatch(p.Id, x))).ToArray());
    }
    ...
    private void CheckAllowedPackages()
    {
        ...
        foreach (var (project, packages) in _projects)
        foreach (var pkg in packages.Where(p => _settings.ErrorSettings.AllowedPackages != null &&
                                                !_settings.ErrorSettings.AllowedPackages.Any(b =>
                                                    b.Id == p.Id &&
                                                    (string.IsNullOrWhiteSpace(b.Version) || VersionRange
                                                        .Parse(p.Version).Satisfies(new(b.Version))))))
        {
            Log.Logger.Error("{packageName}:{version} was included in {nugetFile} but is not in the Allowed List",
                pkg.Id, pkg.Version, _nuGetFile);
    ...
    private void ReportVulnerabilities(Dictionary<string, Dictionary<string, Vulnerability>>? vulnDict)
    {
        if (vulnDict == null)
        {
            Log.Logger.Information("No Vulnerabilities found in {numberOfPackages} packages",
                _projects.Sum(p => p.Value.Length));
        }
        else
        {
            Log.Logger.Verbose("Building report of Vulnerabilities found in {numberOfPackages} packages",
                vulnDict.Keys.Count);
            ...
            foreach (var (project, packages) in _projects)
            {
                ...
                if (_settings.VulnerabilityReports.OutputTextReport)
                    Log.Logger.Information(vulnReporter.VulnerabilityTextReport);
            ...
            if (string.IsNullOrWhiteSpace(_settings.VulnerabilityReports.JsonReportPath) &&
                string.IsNullOrWhiteSpace(_settings.VulnerabilityReports.XmlReportPath)) return;
            ...
            if (!string.IsNullOrWhiteSpace(_settings.VulnerabilityReports.JsonReportPath))
            {
                ...
                File.WriteAllText(_settings.VulnerabilityReports.JsonReportPath.Replace("{project}", _projectFileName),
                    contents
                );
            }
    ...
    private void CheckBlockedPackages()
    {
        ...
        foreach (var pkg in pkgs)
        {
            ...
            var blockedPackage = _settings.ErrorSettings.BlockedPackages.FirstOrDefault(b =>
                b.Package.Id == pkg.Id &&
                (string.IsNullOrWhiteSpace(b.Package.Version) ||
                 VersionRange.Parse(pkg.Version).Satisfies(new(b.Package.Version))));
