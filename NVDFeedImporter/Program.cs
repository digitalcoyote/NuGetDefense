...
namespace NVDFeedImporter
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            var nvdDict =
                new Dictionary<string, Dictionary<string, (string[] versions, string description, string cwe, string
                    vendor, double? score, AccessVectorType vector, string[] references)>>();
            foreach (var link in GetJsonLinks()) AddFeedToNvdDictionary(link, nvdDict);

            SaveToBinFile(nvdDict);
        }

        private static void AddFeedToNvdDictionary(string link,
            Dictionary<string, Dictionary<string, (string[] versions, string description, string cwe, string vendor,
                double? score, AccessVectorType vector, string[] references)>> nvdDict)
        {
            var feed = GetFeed(link);
                ...
                .SelectMany(n => n.CpeMatch))
            {
                ...
                if (!nvdDict.ContainsKey(cpe.Product))
                    nvdDict.Add(cpe.Product,
                        new Dictionary<string, (string[] versions, string description, string cwe, string vendor,
                            double? score, AccessVectorType vector, string[] references)>());
                if (!nvdDict[cpe.Product].ContainsKey(feedVuln.Cve.CveDataMeta.Id))
                {
                    nvdDict[cpe.Product].Add(feedVuln.Cve.CveDataMeta.Id, (
                        ...
                        feedVuln.Cve.References.ReferenceData.Select(r => r.Url.ToString()).ToArray()
                    ));
                }
                else
                {
        ...
        private static JObject GetFeed(string link)
        {
